...
#import "EYPhotoViewerController.h"
#import "EYPhotoBrowserAnimator.h"

@interface EYPhotoBrowserController () <UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIGestureRecognizerDelegate>

@end

@implementation EYPhotoBrowserController {
    EYPhotoBrowserPhotos *_photos;
    BOOL _statusBarHidden;
    EYPhotoBrowserAnimator *_animator;
    
    EYPhotoViewerController *_currentViewer;
...
- (instancetype)initWithSelectedIndex:(NSInteger)selectedIndex urls:(NSArray<NSString *> *)urls parentImageViews:(NSArray<UIImageView *> *)parentImageViews {
    ...
    if (self) {
        _photos = [[EYPhotoBrowserPhotos alloc] init];
        ...
        _animator = [EYPhotoBrowserAnimator animatorWithPhotos:_photos];
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerBeforeViewController:(EYPhotoViewerController *)viewController {
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerAfterViewController:(EYPhotoViewerController *)viewController {
...
- (EYPhotoViewerController *)viewerWithIndex:(NSInteger)index {
    return [EYPhotoViewerController viewerWithURLString:_photos.urls[index] photoIndex:index placeholder:_photos.parentImageViews[index].image];
}

#pragma mark - UIPageViewControllerDelegate
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray<UIViewController *> *)previousViewControllers transitionCompleted:(BOOL)completed {
    
    EYPhotoViewerController *viewer = pageViewController.viewControllers[0];
...
- (void)setPageButtonIndex:(NSInteger)index {
    ...
    NSMutableAttributedString *attributeText = [[NSMutableAttributedString alloc]
                                                initWithString:[NSString stringWithFormat:@"%lu", index + 1]
...
- (void)prepareUI {
    ...
    EYPhotoViewerController *viewer = [self viewerWithIndex:_photos.selectedIndex];
