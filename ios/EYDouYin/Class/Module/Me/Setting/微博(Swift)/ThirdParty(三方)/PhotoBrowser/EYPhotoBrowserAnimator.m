...
@interface EYPhotoBrowserAnimator() <UIViewControllerAnimatedTransitioning>

@end

@implementation EYPhotoBrowserAnimator {
    BOOL _isPresenting;
    EYPhotoBrowserPhotos *_photos;
}

#pragma mark - 构造函数
+ (instancetype)animatorWithPhotos:(EYPhotoBrowserPhotos *)photos {
    return [[self alloc] initWithPhotos:photos];
}

- (instancetype)initWithPhotos:(EYPhotoBrowserPhotos *)photos {
    self = [super init];
    if (self) {
        _photos = photos;
    }
